...
- **v0.2 (2026-09-02 13:36 PDT, Doug's aesthetic ruling):** the editor is **aesthetic-agnostic**. The file gains a per-Georgeland `style` block (preset `standin-flat` / `standin-textured` / `meshes`, per-type mesh `assets` of any style incl. textured PBR GLB/glTF/FBX, a light `budget`); `georgeland_styles.js` renders whatever the style says (stand-in sets, external meshes with their own materials + a PMREM environment) and the status chip shows live triangles / draw calls / asset MB against the budget. Verified: textured preset renders; a skinned PBR GLB (Xbot) instanced at every gold spot at the asked height; the budget line turns red when over. The sample file carries `standin-flat` named “house flat (placeholder until the bake-off)”.
- **Next:** lock the shape to HvM's reply · the house aesthetic from the bake-off goes into `style` · Phase 2 (Vitas, anchors, Conditions) · stage redeploy once merged.
