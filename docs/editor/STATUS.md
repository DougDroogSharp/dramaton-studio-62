...
## NOW
- **Branch state:** `narraton-editor` (14 commits on `2872a41`, then main merged in at `e3408ce` with 390 tests green) **was merged into `main` at `968bd9c` on 2026-09-01** (decision 37, executed by DISPATCH), and its one-Narraton-shape follow-up `be4e24a` was merged at **`c26448a`** on 2026-09-02. `docs/DRAM_BRIDGE.md` is on main. The EDITOR worktree `C:\Users\dougs\editor-lane` tracks `narraton-editor`; fast-forward it to main before the next task.
...
## DECIDE (open, Doug)
1. ~~Two Narraton metadata shapes coexist on Scene~~ — **ruled 2026-09-02, option 2 (Doug, in the EDITOR session): the theater's `Scene.narraton` is the one shape.** Editor `key/phase/subplotId` migrate into `narraton` on load; the editor ranks with `selectNarratonScene`; `narratonDirector.ts` removed; theater runtime unchanged. Branch `be4e24a` (372 tests green) merged into main at `c26448a` by DISPATCH, 2026-09-02 17:03 PDT.
