...
describe('CHOICE options with inline sets (decision points)', () => {
  ...
  it('parses plain options without a sets field', () => {
    const [choice] = parseScript(script) as ChoiceCommand[];
    expect(choice.type).toBe('CHOICE');
    expect(choice.options[0]).toEqual({ text: 'Walk away', target: 'street' });
  });

  it('parses inline sets on an option', () => {
    const [choice] = parseScript(script) as ChoiceCommand[];
    const bribe = choice.options[1];
    expect(bribe.target).toBe('hallway');
    expect(bribe.sets).toHaveLength(2);
    expect(bribe.sets![0]).toMatchObject({ variable: 'guard_trust', op: '+=', value: 10 });
    expect(bribe.sets![1]).toMatchObject({ variable: 'cash', op: '-=', value: 50 });
  });
